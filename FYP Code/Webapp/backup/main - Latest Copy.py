...
@app.route('/')
def home():
    global generate_report, cve_details
    generate_report = 0
    cve_details = {}
...
def start_new_chat():
    try:
        ...
        global current_phase, previous_intent, cve_details, technologies, url, target, generate_report
        current_phase = 1
        previous_intent = None
        cve_details = {}
        technologies = None
...
@app.route('/new_chat')
def new_chat():
    global current_phase, previous_intent, technologies, url, target, generate_report
    current_phase = 1
    previous_intent = None
    technologies = None
    url = None
...
def process():
    ...
    global target, technologies, url
    ...
    if previous_intent == 'target_select' and current_phase == 1:
        ...
        if is_valid_url(url):
            ...
            theme_color = get_theme_color(current_phase)
            return jsonify({'bot_response': "Target " + target + " is not a live target. Do you still want to choose it?", 'previous_intent': previous_intent, 'phase': current_phase, 'theme_color': theme_color})
    ...
    if user_message.lower() == 'exit':
        ...
        if str(current_phase) in phases:
            ...
            # Check if the detected intent has a specific response for the current phase
                ...
                response = f"{phase_responses[detected_intent.lower()]}"
            else:
...
#######################################################################
        ...
        if current_phase == 3 and detected_intent.lower() == "show_exploits":
            try:
                ...
                response += details
                cve_details.update({'Exploit_Details':details})
            except:
        ...
        if current_phase == 3 and detected_intent.lower() == "exploit_details":
            try:
                ...
                response += details
                cve_details.update({'Exploit_Code':exploit_full_code})
            except:
                response += " Unable to print exploits , or either no vulnerability selected!!"

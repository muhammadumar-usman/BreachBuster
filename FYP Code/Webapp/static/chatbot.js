...
function sendMessage() {
    ...
    // Create and append a loading animation element for bot response
    var chatBox = document.getElementById('chat-box');
    var loadingMessage = document.createElement('div');
    loadingMessage.className = 'bot-message loading-animation';
    loadingMessage.innerHTML = '<div class="loading-ring"></div>';
    ...
    $.ajax({
        ...
        error: function(error) {
            ...
            // Remove the loading animation and show error message
            chatBox.removeChild(loadingMessage);
...
window.onload = function() {
    ...
    var greetingMessage = "Hello! Welcome to BreachBuster. I am your personal pentesting assistant. Following 4 steps of pentest i.e Scanning and Reconnaisance, Enummeration, Exploitation, and Remediation & Documnentation, this pentest will be divided into multiple tasks. You are currently in the first phase of pentesting i.e. Scanning and Reconnaisance. Not sure what to do? Simply type 'help'. Let me know how I may assist you in your pentest! ";
...
function simulateTyping(sender, message) {
    ...
    var words = message.split(/\s+/); // Split by whitespace to include line breaks
    var currentWord = 0;

    // Animate writing word by word
    var typingInterval = setInterval(function() {
        if (currentWord < words.length) {
            // Append the current word or handle <br> tags
            if (words[currentWord] === '<br>' || words[currentWord] === '\n') {
                typingMessage.innerHTML += '<br>';
            } else {
                typingMessage.innerHTML += words[currentWord] + ' ';
            }
            currentWord++;
        } else {
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                data.sessions.forEach(session => {
                    ...
                    sessionItem.innerHTML = `<a href="/chathistory/${session[0]}"> ${session[1]}</a>`;
...
// Function to launch Kali in a new tab
function launchKali() {
    // Open a new tab and navigate to the "/webshell" page
    window.open('/webshell', '_blank');
}
